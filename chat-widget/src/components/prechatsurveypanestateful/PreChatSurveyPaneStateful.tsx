...
export const PreChatSurveyPaneStateful = (props: IPreChatSurveyPaneStatefulParams) => {
    ...
    const controlProps: IPreChatSurveyPaneControlProps = {
        ...
        onSubmit: async (values: { index: number, label: any, id: any, value: string }[]) => {
            console.info("PreChatSurveyPaneStateful controlProps onSubmit");
            ...
            try {
                ...
                if (persistedState?.domainStates?.liveChatContext) {
                    ...
                    console.info("setPreChatResponseEmail");
                    console.info(values);
                    setPreChatResponseEmail(values);
                    console.info("initStartChat optionalParams");
    ...
    useEffect(() => {
                        ...
                        for (let index = 0; index < children.length; index++) {
                            ...
                            if (current && current.tagName.toLowerCase() == HtmlAttributeNames.div && current.childElementCount > 0) {
                                ...
                                if (input && input.length > 0 && input[0].className != HtmlAttributeNames.adaptiveCardToggleInputClassName) {
                                    ...
                                    if(input[0].getAttribute(HtmlAttributeNames.ariaLabel) == "fullname")
                                    {
                                        input[0].setAttribute("value",username?username:"");
                                    }else
                                    {                                        
                                        input[0].setAttribute("value",userupn?userupn:"");
                                    }                                    
                                }
                            }
                        }
        ...
        // const buttonElements = document.getElementsByClassName("ac-actionSet");
        // if (buttonElements && buttonElements.length > 0) 
        // {
        //     console.info(buttonElements[0].innerHTML);
        //     const thisbutton = buttonElements[0] as HTMLElement;
        //     thisbutton.focus();
        //     thisbutton.click();
        // }
